...
int cpio_exec_user_program(char *filename) {
    ...
    void *put_addr = (void *)0x200000;  // User program 的記憶體起始位置
    ...
    while (1) {
        ...
        // 2. 讀取文件名稱大小
        int filename_size = atoi(header_ptr->c_namesize, 8);
        int file_size = atoi(header_ptr->c_filesize, 8);        //user_program.S 應該是 799(dec, 用 uart_write_int印出來), 31F(hex，用uart_write_hex印的話)
        uint32_t entry_size = sizeof(struct cpio_newc_header) + filename_size;
        entry_size = (entry_size + 3) & ~3;
        ...
        if (memcmp(current_filename, "TRAILER!!!", filename_size) == 0) {
            ...
            uart_write_str(current_filename);
            uart_write_str("; at ");
            uart_write_hex((uintptr_t)put_addr);
            uart_write_str("; file_size in dec =  ");
            uart_write_int(file_size);


            uint8_t *file_data = (uint8_t *)header_ptr + entry_size;

            // 4. 將 User Program 複製到 `put_addr`
            uint8_t *target = (uint8_t *)put_addr;
            for (int i = 0; i < file_size   ; i++) { //要用十進位的，不然會少搬
                *target = *file_data;
                target++;
                file_data++;
            }
            ...
            //5. 檢查一下有沒有般成功
            target = (uint8_t *)put_addr;
            file_data = (uint8_t *)header_ptr + entry_size;
            for (int i = 0; i < file_size   ; i++) { //要用十進位的，不然會少搬
                if (*target != *file_data){
                    uart_write_str("\r\n **ERROR** user program move fail");
                }
                target++;
                file_data++;
            }
            ...
            asm volatile("msr elr_el1, %0    \n" ::"r"(put_addr));  // 設定 PC 為 User Program 入口點
            asm volatile("msr sp_el0, %0    \n" ::"r"((uintptr_t)put_addr + USTACK_SIZE));  // 設定 User Mode Stack
...
void initramfs_callback(char* name, char* value, struct fdt_property* prop) {
    (void) prop;  // 告訴編譯器 prop 目前沒用，但不要報錯
                    //不直接移除的原因是， fdt_traverse() 是一個通用函數，所有 callback 都要用他
                    //其他人可能會需要用到 prop
    ...
    if (memcmp(name, "linux,initrd-start", 18) == 0) {
        uint32_t initrd_start = fdt32_to_cpu(*(uint32_t*)value);
        ...
        DEVTREE_CPIO_BASE = (uint64_t*)(uintptr_t)initrd_start;
            //uintptr_t 才 64bit save，不能直接轉 uint32_t
        uart_write_str("\r\n    Found initrd_start = ");
        uart_write_hex(initrd_start);
    }else if (memcmp(name, "linux,initrd-end", 16) == 0){
        uint32_t initrd_end = fdt32_to_cpu(*(uint32_t*)value); 
        uart_write_str("\r\n    Found initrd_end = ");
        uart_write_hex(initrd_end);
