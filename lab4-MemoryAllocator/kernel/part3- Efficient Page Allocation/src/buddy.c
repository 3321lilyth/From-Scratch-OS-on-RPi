...
// ---------------- Initialization ----------------
extern char __heap_start ;      //在 linker script 裡面定義
...
void buddy_init() {
    ...
    uart_write_str("\r\n[buddy] Init buddy tree, root set to FREE, heap_start = ");
    uart_write_hex(heap_start_addr);

    //invalid memory
    reserve_memory(0x3C000000, 0x40000000-1);
    //reserved memory
    reserve_memory(0x0000, 0x1000-1);             //spin tables for multicore boot
    //我不想去看 bootloader 的 code 長到哪裡了，還要讓它跟 dtb address 一樣傳過來超麻煩
    //所以 60000~80000...全給啦!! 然後給 kernel heap 保留 1MB、bootloader stack 保留 1000B 這樣
    reserve_memory(0x59000, heap_start_addr + (1024*1024) -1 );             //spin tables for multicore boot

    
    // uart_write_str("\r\nafter init: ");
    // dump_node_info_by_addr(0x0000, -1);
    // dump_node_info_by_addr(0x1000, -1);
    // dump_node_info_by_addr(0x59000, -1);
    // dump_node_info_by_addr(heap_start_addr + (1024*1024), -1);
    // dump_tree_path_to_addr(0x0000, 0);
    // dump_tree_path_to_addr(0x1000, 0);
    // dump_tree_path_to_addr(heap_start_addr + (1024*1024), 0);
