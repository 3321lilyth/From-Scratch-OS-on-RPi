...
void dump_frame_array() {
    ...
    for (int i = 0; i < TOTAL_PAGES; i++) {
        if (frame_array[i].start_of_block == 1){
            ...
            // printf("[%02d] alloc=%d, start=%d, order=%d\n", i,
            //        frame_array[i].allocated,
            //        frame_array[i].start_of_block,
            //        frame_array[i].order);
        }
    }
    uart_write_str("\r\n");
...
void *buddy_alloc(size_t size_in_bytes) {
    //1. 找出最少需要 2^order 個 page 才能滿足這個需求
    ...
    //2. 從小的 order 先開始找，沒空間了再去拆大一點的 order
...
void buddy_free(void *addr) {
    ...
    uart_write_str("\r\n[buddy] freeing, start from index ");
    uart_write_int(index);
