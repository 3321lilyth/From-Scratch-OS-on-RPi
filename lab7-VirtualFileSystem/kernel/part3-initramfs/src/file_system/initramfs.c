...
int initramfs_setup_mount(struct filesystem* fs, struct mount* mount) {
    ...
    mount->fs = fs;
    uart_write_str_raw("\r\n initramfs initramfs_node pointer address = ");
    uart_write_hex_raw((uintptr_t)root_node); //會得到 0x5000 沒問題
    uart_write_str_raw("\r\n initramfs vnode pointer address = ");
    uart_write_hex_raw((uintptr_t)root_vnode); //會得到 0x2040
