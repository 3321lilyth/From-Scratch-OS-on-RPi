...
int main(){
    ...
    thread_create(idle, MAX_PRIORITY - 1, 0);
    thread_create(vfs_test_TA, 0, 1);
    // thread_create(test_initramfs, 0, 1);
    ready_queue_dump();
    
    //8. enable timer & enable EL0 access timer register
    asm volatile("msr tpidr_el1, xzr");
    core_timer_enable();
    set_timeout();

    // 9. check frame buffer
    ...
    //////////////////////////////// 想要直接用這個 kernel thread 跑，不要管 thread 的話藥用這邊，上面兩段 ====== 中間的部分要全部註解
    // test_initramfs();
    // shell();
    return 0;
}
