...
void init_rootfs(){
    uart_write_str_raw("\r\n[init_rootfs] initializing root filesystem and initramfs...");
    //rootfs
    ...
    vfs_mount("/initramfs", "initramfs_fs");

    tmpfs_ls();
    static struct vnode* initramfs_mount_vnode; //這個取出來就是 mount 過後的、新的 FS 的 root，而不會是 tmpfs 的 /initramfs 這個資料夾
    vfs_lookup("/initramfs", &initramfs_mount_vnode);

    initramfs_ls(initramfs_mount_vnode);
}
...
void test_initramfs() {
    // uart_write_str_raw("\r\n\r\n====== [TEST0] Mount initramfs on /initramfs ======");

    // // 先用 tmpfs 建立掛載點 /initramfs
    // vfs_mkdir("/initramfs");
    // register_filesystem(&initramfs_fs);
    // vfs_mount("/initramfs", "initramfs_fs"); //前面是路徑後面是檔案系統名稱

    // // Step 1: dump initramfs ls 結構
    // uart_write_str_raw("\r\n\r\n====== [TEST1] initramfs_ls() ======");
    // static struct vnode* initramfs_mount_vnode; //這個取出來就是 mount 過後的、新的 FS 的 root，而不會是 tmpfs 的 /initramfs 這個資料夾
    // vfs_lookup("/initramfs", &initramfs_mount_vnode); //這是 0x2040
    // //注意 vfs_lookup 得到的是 tmpfs 的 /initramfs 資料夾的 vnode address !!!!
    // //但是實際上要傳入到 FS ls 的必須要是該FS的 root，而不是他掛載的地方
    // // 所以還會需要用 mount->root 去得到正確的 vnode
    // uart_write_str_raw("\r\n    [TEST] address from vfs_lookup, initramfs_mount_vnode = ");
    // uart_write_hex_raw((uintptr_t)initramfs_mount_vnode); //這是 0x2020
    // uart_write_str_raw("\r\n    [TEST] initramfs_mount_vnode->mount = ");
    // uart_write_hex_raw((uintptr_t)initramfs_mount_vnode->mount); //這是0x1010
    // uart_write_str_raw("\r\n    [TEST] initramfs_mount_vnode->mount->root = ");
    // uart_write_hex_raw((uintptr_t)initramfs_mount_vnode->mount->root); //這是 0x2040
    // initramfs_ls(initramfs_mount_vnode);




    // Step 2: 測試 lookup 和 read
    uart_write_str_raw("\r\n\r\n====== [TEST2] lookup + read ======");
    static struct vnode* target;
    if (vfs_lookup("/initramfs/file1", &target) == 0) {
        static struct file* f;
        static int open_ret;
        open_ret = vfs_open("/initramfs/file1", 0, &f);
        if (open_ret != 0) {
            uart_write_str_raw("\r\n[TEST] open failed, error = ");
            uart_write_str_raw(fs_strerror(open_ret));
        }else{
            static char buf[48] = {0};
            static int read_len;
            read_len = vfs_read(f, buf, sizeof(buf) - 1);
            uart_write_str_raw("\r\n[TEST] read content (len = ");
            uart_write_int_raw(read_len);
            uart_write_str_raw("):\r\n");
    ...
    // Step 3: 測試 write 應該失敗
    uart_write_str_raw("\r\n\r\n====== [TEST3] write not allowed ======");
    static struct file* f2;
    vfs_open("/initramfs/file1", 0, &f2);
    static int write_ret;
    write_ret = vfs_write(f2, "fail", 4);
    uart_write_str_raw("\r\n    [TEST] write result (expect error): ");
    uart_write_str_raw(fs_strerror(write_ret));
    vfs_close(f2);

    // Step 4: 測試 create/mkdir 都要失敗
    uart_write_str_raw("\r\n\r\n====== [TEST4] create/mkdir not allowed ======");
    static int create_ret;
    create_ret = vfs_create("/initramfs/new_file.txt");
    static int mkdir_ret;
    mkdir_ret = vfs_mkdir("/initramfs/new_dir");

    uart_write_str_raw("\r\n    [TEST] vfs_create result: ");
    uart_write_str_raw(fs_strerror(create_ret));
    uart_write_str_raw("\r\n    [TEST] vfs_mkdir result: ");
    uart_write_str_raw(fs_strerror(mkdir_ret));
    while(1);
}