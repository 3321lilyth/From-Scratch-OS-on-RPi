...
void syscall_handler(uint64_t* sp) {
    ...
    if (syscall_num != SYS_UART_READ && syscall_num !=SYS_UART_WRITE){
        ...
        case SYS_GETPID:
            ...
            case SYS_SIGRETURN:
                ...
                break;
            case SYS_OPEN:
                uart_write_str_raw(" (SYS_OPEN)");
                break;
            case SYS_CLOSE:
                uart_write_str_raw(" (SYS_CLOSE)");
                break;
            case SYS_WRITE:
                uart_write_str_raw(" (SYS_WRITE)");
                break;
            case SYS_READ:
                uart_write_str_raw(" (SYS_READ)");
                break;
            case SYS_MKDIR:
                uart_write_str_raw(" (SYS_MKDIR)");
                break;
            case SYS_MOUNT:
                uart_write_str_raw(" (SYS_MOUNT)");
                break;
            case SYS_CHDIR:
                uart_write_str_raw(" (SYS_CHDIR)");
                break;
            default:
    ...
    // if (syscall_num >=11) {
    //     uart_write_str_raw("\r\n\r\n------------------------");
    //     extern void tmpfs_ls(); // for debug
    //     tmpfs_ls(); // for debug
    //     uart_write_str_raw("\r\n------------------------\r\n");
    // }
    switch (syscall_num) {
        ...
        case SYS_EXEC: { //3 板子OK
            ...
            if (is_func == 0){
                sp[0] = cpio_exec_user_program_lab7(name_or_func);
            }else{
        ...
        case SYS_SIGRETURN: {  // 10
            ...
            if (t->handling_signal) {
                uart_write_str_raw("\r\n[syscall] sigreturn: restoring context");
        ...
        case SYS_CLOSE: {
            int fd = sp[0];
            sp[0] = vfs_close_fd(fd);
            break;
        }
        case SYS_WRITE: {
            int fd = sp[0];
            const void* buf = (const void*)sp[1];
            size_t count = sp[2];
            sp[0] = vfs_write_fd(fd, buf, count);
            break;
        }
        case SYS_READ: {
            int fd = sp[0];
            void* buf = (void*)sp[1];
            size_t count = sp[2];
            sp[0] = vfs_read_fd(fd, buf, count);
            break;
        }
        case SYS_MKDIR: { // mkdir
            const char* path = (const char*)sp[0];
            sp[0] = vfs_mkdir(path);
            break;
        }
        case SYS_MOUNT: { // mount
            const char* target = (const char*)sp[1];
            const char* fs_name = (const char*)sp[2];
            sp[0] = vfs_mount(target, fs_name);
            break;
        }
        case SYS_CHDIR: {
            const char* path = (const char*)sp[0];
...
int mount(const char *src, const char *target, const char *filesystem, unsigned long flags, const void *data) {
    ...
    asm volatile (
        ...
        "mov %0, x0\n"
        : "=r" (ret)
        : "r" (src), "r" (target), "r" (filesystem), "r" (flags), "r" (data), "i" (SYS_MOUNT)
        : "x0", "x1", "x2", "x3", "x4", "x8"
    );
    return ret;
}
