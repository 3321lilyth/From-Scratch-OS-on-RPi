...
//這是讓 exec 自己直接到 initramfs.cpio 把程式碼複製出來，但是有了 lab7 之後應該要用 VFS API 來取得程式碼 
...
int cpio_exec_user_program_lab7(const char *filename) {
    ...
    // === Phase 1: 預讀算大小 ===
    int fd = vfs_open_fd(filename, 0);
    if (fd < 0) {
        uart_write_str_raw("\r\n[exec_vfs] ERROR: file not found: ");
        uart_write_str_raw(filename);
        return -1;
    }
    ...
    char temp_buf[256];
    int len;
    while ((len = vfs_read_fd(fd, temp_buf, sizeof(temp_buf))) > 0) {
        total_size += len;
    }
    vfs_close_fd(fd);

    if (total_size == 0) {
        uart_write_str_raw("\r\n[exec_vfs] ERROR: file empty");
        return -1;
    }

    // === Phase 2: 重新 open 並讀入內容 ===
    fd = vfs_open_fd(filename, 0);
    if (fd < 0) {
        uart_write_str_raw("\r\n[exec_vfs] ERROR: reopen failed");
        return -1;
    }

    void *user_code_base = kmalloc(total_size);
    if (!user_code_base) {
        uart_write_str_raw("\r\n[exec_vfs] ERROR: kmalloc failed");
        vfs_close_fd(fd);
        return -1;
    }
    ...
    size_t offset = 0;
    while ((len = vfs_read_fd(fd, temp_buf, sizeof(temp_buf))) > 0) {
        simple_memcpy((char *)user_code_base + offset, temp_buf, len);
        offset += len;
    }
    vfs_close_fd(fd);

    // === 清空舊的堆疊與 fd table ===
    //我看助教的 BASIC 3 測試應該是預期這邊不能被清空
    memset(current->user_stack_base, 0, THREAD_STACK_SIZE);
    ...
    // === 設定新的 trap frame ===
    uint64_t *kernel_sp = (uint64_t *)current->kernel_sp;
    kernel_sp[30] = (uint64_t)user_code_base;                  // x30
    kernel_sp[32] = 0x340;                                     // spsr_el1
    kernel_sp[35] = 1;                                         // is_user
    kernel_sp[33] = (uint64_t)user_code_base;                  // elr_el1
    kernel_sp[34] = (uint64_t)current->user_stack_base + THREAD_STACK_SIZE - 16; // sp_el0
    current->user_sp = kernel_sp[34];
    ...
    current->signal_stack_base = 0;
    current->handling_signal = 0;
    //我看助教的程式碼好像沒有重設定這個部分，導致我的 BASIC3 測試失敗
    extern struct mount* rootfs;
    current->cwd = rootfs->root;

    // === debug log ===
    uart_write_str_raw("\r\n[exec_vfs] Loaded user program via VFS: ");
    uart_write_str_raw(filename);
    uart_write_str_raw(", at ");
    uart_write_hex_raw((uintptr_t)user_code_base);
    uart_write_str_raw(", total size = ");
    uart_write_int_raw(total_size);
    uart_write_str_raw(", tid = ");
    uart_write_int_raw(current->tid);
    uart_write_str_raw(", user_sp = ");
    uart_write_hex_raw((uintptr_t)current->user_sp);
