...
void print_help(){
    if (uart_interrupt_enabled == 0){
        ...
        uart_write_str("\r\n    ------------------- thread related -------------------------");      
        uart_write_str("\r\n    thread_foo                : lab5 part1 thread test case");
    }else{
...
void foo_async(){
    for(int i = 0; i < 10; ++i) {
        uart_write_str_async("\r\n[foo] Thread id: ");
        uart_write_int_async(get_current()->tid);
        uart_write_str_async(", #");
        uart_write_int_async(i);
        

        //印一堆垃圾字串
        // for (int j = 0; j < 600; ++j) {
        //     uart_write_char_async('A' + (j % 26));  // ABC...ZABC...
        // }

        // uart_write_str_async("\r\n    write waiting queue: ");
        // thread_t* cur = uart_tx_lock.head;
        // while (cur) {
        //     uart_write_str_async("tid=");
        //     uart_write_int_async(cur->tid);
        //     uart_write_str_async(" ");
        //     cur = cur->next;
        // }
        sleep(100);       //說 linux 是 5-10ms 換一次 thread，這邊放太小就會卡死
    }
    thread_exit();
}
...
void shell(){
	while(1){
        ...
        // else if (strcmp(cmd, "thread_foo_async")){
        //     uart_write_str("\r\n=========================  create foo_async threads  =========================");
        //     for(int i=0; i<5; i++){
        //         thread_create(foo_async, 0);
        //     }
        //     uart_write_str("\r\n========================= after thread create =========================");
        //     async_on();
        //     schedule();
