...
/// helper function
extern void switch_to_first(thread_t* next);
...
//init=1 代表接下來要執行的是第一個 thread，這是為了避免 tpidr_el1 沒有初始化的問題
void schedule(int init) {
    disable_interrupt();
    validate_ready_queue();

    thread_t* prev = init ? 0 : get_current();
    thread_t* next = 0;

    // 依照 priority 尋找下一個 READY thread
    for (int prio = 0; prio < MAX_PRIORITY; prio++) {
        thread_t* cur = ready_queue[prio];
        while (cur) {
    ...
    if (next != prev) {
        ...
        uart_write_int(prev ? prev->tid : 0);
