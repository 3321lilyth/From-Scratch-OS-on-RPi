...
int thread_exec_user_function(void (*func)()) {
    ...
    // Reset signal handlers and pending signals
...
int thread_send_signal(int target_tid, int signum) {
    disable_interrupt();
    thread_t* target = find_thread_by_tid(target_tid);  // 你應該已有這樣的 helper
    if (!target) {
        enable_interrupt();
        return -1;
    }
        ...
        if (next->signals.pending[i]) {
            ...
            if (i == SIGKILL && !next->signals.handlers[i]) {
                ...
                if (next->signal_stack_base == 0) {
                    void* handler_stack = kmalloc(THREAD_STACK_SIZE);
                    next->signal_stack_base = (uint64_t)handler_stack;
                }
                ...
                tf[34] = next->signal_stack_base + THREAD_STACK_SIZE - 16;   // sp_el0
...
void schedule_preempt() {
    ...
    if (prev->status == THREAD_READY) {
        ...
        uart_write_str_raw("\r\n[DEBUG] skip move_to_ready_queue_tail because prev is not READY, tid=");
        uart_write_int_raw(prev->tid);
    }
    // move_to_ready_queue_tail(prev);
    thread_t* next = choose_next();
