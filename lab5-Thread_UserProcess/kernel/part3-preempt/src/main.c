...
void shell(){
	while(1){
        ...
        // } 
            ...
            thread_create(idle, MAX_PRIORITY - 1);
            for(int i=0; i<5; i++){
                thread_create(foo_sync_preempt, 0);
            }
            uart_write_str("\r\n========================= after thread create =========================");
            ready_queue_dump();
            asm volatile("msr tpidr_el1, xzr");
        ...
        else if (strcmp(cmd, "foo_async_preempt")){
            uart_write_str("\r\n=========================  create foo_async_preempt threads  =========================");
            thread_create(idle, MAX_PRIORITY - 1);
            for(int i=0; i<5; i++){
                thread_create(foo_async_preempt, 0);
            }
            uart_write_str("\r\n========================= after thread create =========================");
            ready_queue_dump();
            asm volatile("msr tpidr_el1, xzr");
