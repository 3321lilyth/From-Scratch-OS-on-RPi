...
static void print_range(uintptr_t addr, int order) {
    ...
    uart_write_str_raw(", Range of pages: [");
    uart_write_int_raw(start);
    uart_write_str_raw(", ");
    uart_write_int_raw(end);
    uart_write_str_raw("]");
}
...
void buddy_free(void *ptr) {
    ...
    if (node_index < 0) {
        uart_write_str("\r\n    [ERROR] buddy free failed: address not found in tree, addr = ");
        uart_write_hex(addr);
        return;
    }
