...
void shell(){
	while(1){
        ...
        //memory related
            ...
            char *endptr;
            uintptr_t addr = (uintptr_t)strtoull(cmd + 5, &endptr, 0);
            if (*endptr != '\0') {
...
int main(){
    ...
    extern uintptr_t dtb_start_addr; // lab2 part6，注意從 bootloader 拿到的應該是 PA，你要把它轉為 VA 才可以存到變數裡面
    asm volatile("mov %0, x21" :  "=r"(dtb_start_addr));
    dtb_start_addr = PA_TO_VA_KERNEL(dtb_start_addr);
    ...
    //6. mrelocate_sp_to_high_va + setting mmu
    register uintptr_t old_sp;
    asm volatile("mov %0, sp" : "=r"(old_sp));  // 把舊的 sp 寄存器存到變數

    uintptr_t new_sp = old_sp | 0xffff000000000000UL;  // 假設 high VA offset 為 0xffff000000000000
    asm volatile("mov sp, %0" : : "r"(new_sp));        // 設定 sp 為對應 high VA
    kernel_vm_init();
    ...
    // thread_create(idle, MAX_PRIORITY - 1, 0, "");
    // thread_create(0, 0, 1, "syscall.img");   // for lab5
    thread_create(0, 0, 1, "vm.img");           // for lab6
    // ready_queue_dump();
    ...
    uart_write_str("\r\n============================= start running tasks =============================");
    //8.開始跑囉，如果要跑我們自己的 shell 就要把 schedule_complete((void*)1); 先註解掉喔!!!
    schedule_complete((void*)1);
    // shell();
    return 0;
}
