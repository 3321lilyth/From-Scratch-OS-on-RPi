...
int current_priority = 999;  // 越小越高，999 表示空閒中
int executing_task = 0;
...
void lower_sync_handler(unsigned long type, unsigned long esr, unsigned long elr, unsigned long spsr, unsigned long far){
    disable_interrupt();
    ...
    uart_write_str_raw("\r\nTTBR0_EL1 ");
    uart_write_hex_raw(ttbr0);
    uart_write_str_raw("\r\nTTBR1_EL1 ");
    uart_write_hex_raw(ttbr1);
