...
int mailbox_call(unsigned char channel, unsigned int *mbox_PA){
    ...
    unsigned int msg = ((unsigned int)((unsigned long)mbox_PA) & ~0xF) | (channel & 0xF);
...
int init_framebuffer(framebuffer_info_t *fb_info, uint32_t width, uint32_t height, uint32_t depth, uint32_t pixel_order) {
    ...
    if (mailbox_call(8, mbox)) {
        if (mbox[20] != 0) {
            ...
            fb_info->buffer = (uint8_t*)PA_TO_VA_KERNEL((uintptr_t)(mbox[24] & 0x3FFFFFFF));  
...
void draw_test_pattern() {
    ...
    for (uint32_t y = 0; y < height; y++) {
        uint8_t* fb_va = (uint8_t*)PA_TO_VA_KERNEL((uintptr_t)fb_addr);
        pixel = (uint16_t *)(fb_va + y * pitch);
