...
//這段 code 在 qemu 上面會動，但是上板子就不會動，要改成下面的 set_gpio_uart() 版本才會動....
...
void uart_init(){
    ...
    *AUX_MU_IER = 0x03;        // enable interrupt (0x01 = RX interrupt) (0x02 = TX interrupt)
    ...
    *AUX_MU_IIR = 0xc6;        // enable FIFO, 是一種緩衝機制，比如 read 的時候會先寫到 FIFO，CPU再去從 FIFO撈數據，可以提高數據傳輸效率、防止數據丟失
    ...
    //set GPIO 14,15 為 ALT5 (011)
    // set_gpio_alt(14, 5);
    // set_gpio_alt(15, 5);
	set_gpio_uart();
	
    //disable GPIO pull
    gpio_disable_pull(14);
    gpio_disable_pull(15);
	
    //after config, enable Tx, Rx
    *AUX_MU_CNTL = 3;
	// Enable the interrupts globally (later will add interrupt handler)
