...
void parse_settimeout(char *cmd) {
    ...
    // 呼叫 timer 插入函式
    add_timer(timeout_callback, msg, seconds);
}
...
void timeout_callback(void *msg) {
    uart_write_str("\r\n[TIMEOUT] ");
...
void add_timer(void (*callback)(void*), const char *msg, unsigned int seconds) {
    // 1. 找空 slot
    int idx = -1;
    ...
    // 2. 複製字串到 msg pool
    simple_strncpy(timer_msg_pool[idx], msg, MAX_MSG_LEN);
...
void core_timer_handler() {
    uart_write_str("\r\n[Core Timer] into core_timer_handler");
    ...
    if (timer_queue) {
        ...
        //雖然前面的迴圈理論上已經處理掉這種 case，但是有可能會有邊緣 C\\case
