...
void c_write_handler() {
    if (write_buf_start != write_buf_end) {
        ...
        if (c == '\n') {
            while (!((*AUX_MU_LSR) & 0x20)) asm volatile("wfe");
            *AUX_MU_IO = '\r';
        }
...
void c_read_handler(){
    ...
    read_buf_end = read_buf_end % BUFFER_MAX_SIZE;

    // 確保 TX IRQ 開啟
    //為甚麼 read cmd 的時候要開啟 write? 是因為當使用者輸入時我們希望他打的字同步出現在螢幕上
    //所以 read 的同時才會需要 enable write
    *AUX_MU_IER |= 0x02;
}
...
char uart_read_async() {
    while (read_buf_start == read_buf_end) {  // 等待緩衝區有數據
        asm volatile("wfe");
    }
...
void uart_write_char_async(char c) {
    ...
    //如果這邊沒有等的話 kernel 輸出會更快，真的到螢幕上的輸出會更少
...
void uart_write_str_async(const char *str){
    while (*str) {
        if (*str == '\n') uart_write_char_async('\r');  // 處理 CRLF
        uart_write_char_async(*str++);
    }
}
